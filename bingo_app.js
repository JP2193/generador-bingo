...
function buildCellsInline(frases, img5b64) {
  ...
  for (let i = 0; i < 25; i++) {
    if (i === 12) {
      cells.push(`<div style="display:flex;align-items:center;justify-content:center;padding:4px;background:#fdfaf5;border:0.5px solid rgba(140,100,60,0.75);border-radius:9px;overflow:hidden;margin:2px;"><img src="${img5b64}" style="max-width:88%;max-height:88%;width:auto;height:auto;display:block;filter:contrast(1.15) saturate(1.1);"></div>`);
    } else {
...
function buildSheetDOM(grupo, img4b64, img5b64, img6b64) {
  ...
  const cartones = grupo.map(frases => `
    <div style="position:relative;display:flex;flex-direction:column;width:${cW}px;height:${cH}px;background:#fdfaf5;overflow:hidden;border:1px solid #dfd0b9;">
      ...
      <div style="display:grid;grid-template-columns:repeat(5,1fr);grid-template-rows:repeat(5,1fr);flex:1;background:transparent;gap:0;margin:5px 8px ${gridMarginB - 5}px;position:relative;z-index:1;">
